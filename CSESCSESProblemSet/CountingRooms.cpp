...
void DFS(V<V<int>> &vis, V<string> &t, int x, int y) {
  vis[x][y] = 1;
  rep (4) {
    int new_x = x + X[i];
    int new_y = y + Y[i];
    if (CheckBounds(vis, new_x, new_y) && !vis[new_x][new_y] && t[new_x][new_y] != '#') {
      DFS(vis, t, new_x, new_y);
    }
  }
}
...
int solve() {
  read(int, n, m);
  auto t = read_vector<string>(n);
  auto vis = make_2D<int>(n, m);
...
/* ================= Notes ================== //
  Problem asks me to count # of connected components

  *made changes, update template
*/
