...
int solve() {
  read(llong, n, t);
  M<llong, llong> player_score; // [player, score]
  M<llong, llong> score_cnt; // [score, # of times it appears]
  ...
  llong new_players = 0;
  llong ans = 0; // distinct # of scores in score_cnt
  rep (t) {
    read(llong, a, b);
    if (player_score[a] == 0) new_players++;

    score_cnt[player_score[a]]--;
    if (score_cnt[player_score[a]] == 0) ans--;
    
    player_score[a] += b;
    
    score_cnt[player_score[a]]++;
    if (score_cnt[player_score[a]] == 1) ans++;

    print(ans + (new_players < n));
  }
...
/* ================= Notes ================== //
    ...
    * add 1 in case atleast one player is at 0 score
*/
